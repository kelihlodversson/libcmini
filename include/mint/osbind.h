 ...
 *
 * The trap_<N>_<letters> primitives these bindings are built on (function
 * number + arguments -> a GEMDOS/BIOS/XBIOS call) are architecture
 * specific -- see arch/m68k/osbind.h and arch/arm/osbind.h for why the
 * two look so different (m68k pushes arguments of varying width onto the
 * stack; pTOS/ARM passes them in fixed 32-bit registers) -- but the
 * GEMDOS/BIOS/XBIOS function definitions below are written purely in
 * terms of those primitives, so they don't need to know the difference
 * and stay shared between architectures.
 */
