...
#
# Configuration: obj/auto.conf, generated by "make <name>_defconfig" or
# "make menuconfig" from the Kconfig tree, sets ARCH_M68K/ARCH_ARM and the
# ONLY_68K/BUILD_*/COMPILE_ELF/STDIO_* options below (see Kconfig). If it
# doesn't exist -- no configuration has been chosen yet -- fall back to
# building every m68k multilib variant as a.out, matching this Makefile's
# behavior before the Kconfig migration, so a bare "make" still works.
#
-include obj/auto.conf

ifeq ($(wildcard obj/auto.conf),)
ONLY_68K=N
BUILD_CF=Y
